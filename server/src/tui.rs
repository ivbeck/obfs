//! Terminal UI for the server.
//!
//! Top pane: listen address + active client count + cert info. Middle pane:
//! aggregate TX/RX counters and per-second throughput. Bottom pane: recent
//! log lines from the in-memory tracing ring. Quit with `q` / `Esc` /
//! `Ctrl+C`.
...
use vpn_obfs_common::observe::{fmt_bytes, LogRing, PumpStats, StatsSnapshot};

pub struct ServerTuiCtx {
    pub listen: String,
    pub domain: String,
    pub subnet: String,
...
pub fn run(ctx: ServerTuiCtx) -> io::Result<()> {
...
fn event_loop<B: Backend>(terminal: &mut Terminal<B>, ctx: &ServerTuiCtx) -> io::Result<()> {
...
fn render(f: &mut ratatui::Frame, ctx: &ServerTuiCtx, tput: &Throughput) {
    let area = f.area();
    ...
    let info_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::raw(&ctx.cert_source),
        ]),
        Line::from(vec![
            Span::styled("Active : ", Style::default().fg(Color::Gray)),
            Span::styled(
                format!("{active} client{}", if active == 1 { "" } else { "s" }),
                Style::default()
                    .fg(if active > 0 {
                        Color::Green
                    } else {
    ...
    let footer = Paragraph::new(Line::from(Span::styled(
        format!(
            "active: {}  tx pkts: {}  rx pkts: {}",
            active, snap.tx_packets, snap.rx_packets
        ),
